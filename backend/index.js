...
// Root endpoint
app.get("/", (req, res) => {
  res.json("Hello");// Load environment variables and modules
require("dotenv").config();
const cors = require("cors");
const express = require("express");
const fileupload = require("express-fileupload");
...
// API route mounting
const uploadRoutes = require("./routes/FileUpload");   // File upload routes
const authRoutes = require("./routes/userRoute");      // Standard user authentication routes
const oauthRoutes = require("./routes/authRoutes");    // OAuth routes (Google, GitHub, Twitter)
...
// Mount API routes under /api/auth for consistency
app.use("/api/auth", authRoutes);          // Standard auth routes
app.use("/api/auth/upload", uploadRoutes); // File upload routes
app.use("/api/auth", oauthRoutes);          // OAuth routes (Google, GitHub, Twitter)
